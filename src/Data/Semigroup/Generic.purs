...
instance genericSemigroupProduct ::
  ( GenericSemigroup a
  , GenericSemigroup b
  ) =>
  GenericSemigroup (Product a b) where
  genericAppend' (Product a1 b1) (Product a2 b2) =
    Product (genericAppend' a1 a2) (genericAppend' b1 b2)

instance genericSemigroupConstructor ::
  GenericSemigroup a =>
  GenericSemigroup (Constructor name a) where
  genericAppend' (Constructor a1) (Constructor a2) = Constructor
    (genericAppend' a1 a2)

instance genericSemigroupArgument ::
  Semigroup a =>
  GenericSemigroup (Argument a) where
...
genericAppend
  :: forall a rep. Generic a rep => GenericSemigroup rep => a -> a -> a
