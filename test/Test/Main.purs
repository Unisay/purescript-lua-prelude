...
testOrd x y ord =
  assert
    ( "(compare " <> show x <> " " <> show y <> " ) is not equal to " <> show
        ord
    )
    $ (compare x y) == ord
    ...
    in
      do
        assert (msg <> "Quotient/remainder law") $
          q * b + r == a
        assert
          (msg <> "Remainder should be between 0 and `abs b`, got: " <> show r)
          $
            0 <= r && r < abs b
...
testRecordInstances = do
  ...
  assert "Record +" $ ({ a: 1, b: 2.0 } + { a: 0, b: (-2.0) }) ==
    { a: 1, b: 0.0 }
  assert "Record *" $ ({ a: 1, b: 2.0 } * { a: 0, b: (-2.0) }) ==
    { a: 0, b: -4.0 }
  ...
  assert "Record append" $ { a: [], b: "T" } <> { a: [ 1 ], b: "OM" } ==
    { a: [ 1 ], b: "TOM" }
...
testReflectType = do
  assert "reflectType: Symbol -> String" $ reflectType (Proxy :: _ "erin!") ==
    "erin!"
  assert "reflectType: Boolean -> Boolean, True" $ reflectType (Proxy :: _ True)
    == true
  assert "reflectType: Boolean -> Boolean, False" $
    reflectType (Proxy :: _ False) == false
  assert "reflectType: Ordering -> Ordering, LT" $ reflectType (Proxy :: _ LT)
    == LT
  assert "reflectType: Ordering -> Ordering, GT" $ reflectType (Proxy :: _ GT)
    == GT
  assert "reflectType: Ordering -> Ordering, EQ" $ reflectType (Proxy :: _ EQ)
    == EQ
...
testReifyType = do
  assert "reifyType: String -> Symbol" $ reifyType "erin!" reflectType ==
    "erin!"
  assert "reifyType: Boolean -> Boolean, true" $ reifyType true reflectType ==
    true
  assert "reifyType: Boolean -> Boolean, false" $ reifyType false reflectType ==
    false
...
testSignum = do
  assert "Clarifies what 'signum positive zero' test is doing" $
    show (1.0 / 0.0) == "Infinity"
  assert "signum positive zero" $ show (1.0 / (signum 0.0)) == "Infinity"
  assert "Clarifies what 'signum negative zero' test is doing" $
    show (1.0 / (-0.0)) == "-Infinity"
